...
if [ "$HOSTNAME" = darcy ]; then
    ...
    rq run -R -J 'big-lj-benchmark' target/release/lj-cluster --N 31 --min-allowed-energy=-133.53 --max-allowed-energy=-110 --translation-scale 0.03 --movie-time '10^(1/8)' --save-time 1 --radius 5 --energy-bin 0.001 --save-as big-lj-benchmark.yaml --Inv-t-WL
