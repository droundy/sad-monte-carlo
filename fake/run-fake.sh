...
cargo build --release --bin replicas --bin binning
...
rq run -c all --max-output=30 -R -J r-linear -- ${REP[@]//} --save-as r-linear.yaml --min-T 0.001 --fake-linear

rq run -c all --max-output=30 -R -J r-quadratic -- ${REP[@]//} --save-as r-quadratic.yaml --min-T 0.001 --fake-quadratic-dimensions 3

rq run -c all --max-output=30 -R -J r-pieces -- ${REP[@]//} --save-as r-pieces.yaml --min-T 0.001 ${PIECES[@]//}
...
for de in 0.001 0.01 0.1; do
  ...
  rq run -R -J sad-pieces-$de -- ${SAD[@]//} --histogram-bin $de ${PIECES[@]//} --save-as sad-pieces-$de.yaml --sad-min-T 0.001
done
...
# rq run -R -J t-pieces -- ${TRA[@]//} ${PIECES[@]//} --save-as t-pieces.yaml --min-T 0.001 --f '0.5'
