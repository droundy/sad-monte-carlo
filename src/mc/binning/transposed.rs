...
pub struct Bins {
    min_e: Energy,
    max_e: Energy,
    /// The fractional acceptance per bin.
    pub f: f64,
    /// The energy for each energy bin.
    pub energies: Vec<Energy>,
    /// Histogram information for each bin
    pub counts: BinCounts,
    /// The extra totals
...
impl Binning for Bins {
    ...
    fn increment_count(&mut self, e: Energy, gamma: f64) {
        ...
        self.counts.increment_count(e, idx, gamma);
    }
    ...
    fn count_states<F: Fn(Energy, PerEnergy) -> bool>(&self, f:F) -> usize {
        let mut total = 0;
        for i in 0..self.counts.count.len() {
    ...
    fn get_count(&self, e: Energy) -> PerEnergy {
        let idx = self.energy_to_index(e);
        self.counts.get_count(idx) as f64/self.width
    }
    fn max_lnw(&self) -> f64 {
        0.0
    }
    fn min_lnw(&self) -> f64 {
        -self.f*self.energies.len() as f64
    }
    fn max_count(&self) -> PerEnergy {
        self.counts.max_count as f64/self.width
    }
