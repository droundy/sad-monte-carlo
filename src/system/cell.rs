...
impl Cell {
    ...
    pub fn verify_maybe_interacting_includes_everything(&self, r1: Vector3d<Length>) {
        ...
        let mi: Vec<_> = self
            .maybe_interacting_atoms(r1)
            .map(|r| self.put_in_cell(r))
            .collect();
        let d2s: Vec<_> = self
            .maybe_interacting_atoms(r1)
            .map(|r| (r - r1).norm2())
        ...
        for &r2 in self.positions.iter() {
            ...
            if dist2 <= self.well_width * self.well_width {
                if !mi.iter().any(|&rrr| {
                    self.closest_distance2(rrr, r2) < 1e-10 * units::SIGMA * units::SIGMA
                }) {
                    println!("");
                    println!(
                        "  subcell is {} vs {}",
    ...
    pub fn verify_maybe_interacting_excluding_includes_everything(
        ...
        self.verify_subcells_include_nothing_out_of_place();
        let mi: Vec<_> = self
            .maybe_interacting_atoms_excluding(r1, exc)
            .map(|r| self.put_in_cell(r))
            .collect();
        let d2s: Vec<_> = self
            .maybe_interacting_atoms_excluding(r1, exc)
        ...
        for &r2 in self
            .positions
            .iter()
            .filter(|&&r2| r2 != self.positions[exc])
        {
            let dist2 = self.closest_distance2(r1, r2);
            if dist2 <= self.well_width * self.well_width {
                println!("==--> {} from {}", dist2, r2);
                for rr in mi.iter().map(|&r| r).filter(|&rrr| {
                    self.closest_distance2(rrr, r2) < 1e-10 * units::SIGMA * units::SIGMA
                }) {
                    println!("    ==--> {}", rr);
                }
                if !mi.iter().any(|&rrr| {
                    ...
                    println!("  dist2 = {}", dist2);
                    println!(
                        "      mi contains {} with this distance",
                        d2s.iter().filter(|&&d| d == dist2).count()
                    );
                    println!(
                        "      XX contains {} with this distance",
                        self.positions
                            .iter()
                            .filter(|&&r| self.closest_distance2(r1, r) == dist2)
                            .count()
                    );
