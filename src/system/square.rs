...
impl System for SquareWell {
    ...
    fn compute_energy(&self) -> Energy {
        ...
        for (i, &r1) in self.positions[..self.positions.len() - 1]
            .iter()
            .enumerate()
        {
...
impl GrandSystem for SquareWell {
    fn plan_add(&mut self, rng: &mut MyRng) -> Option<Energy> {
        let r = self.put_in_cell(Vector3d::new(
            Length::new(rng.sample(Uniform::new(0.0, self.box_diagonal.x.value_unsafe))),
...
impl From<SquareWellNParams> for SquareWell {
    fn from(params: SquareWellNParams) -> SquareWell {
        ...
        let dim: CellDimensions = match params._dim {
            ...
            CellDimensionsGivenNumber::FillingFraction(f) => CellDimensions::CellVolume(
                (n as f64) * (PI * units::SIGMA * units::SIGMA * units::SIGMA / 6.0) / f,
            ),
        };
        ...
        // y, and z dimensions
        let cells = [
            *(sw.box_diagonal.x / min_cell_width).value() as usize,
        ...
        let cell_width = [
            sw.box_diagonal.x / cells[0] as f64,
        ...
        let offset = [
            Vector3d::new(Length::new(0.0), Length::new(0.0), Length::new(0.0)),
...
fn closest_distance_matches() {
    ...
    for &r1 in sw.positions.iter() {
        for &r2 in sw.positions.iter() {
            assert_eq!(
                sw.closest_distance2(r1, r2),
                sw.unsafe_closest_distance2(r1, r2)
            );
            assert_eq!(
                sw.closest_distance2(r1, r2),
                sw.sloppy_closest_distance2(r1, r2)
            );
        }
    }
